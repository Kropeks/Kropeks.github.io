...
class RecipeAPI {
  ...
  async getNutritionInfo(query) {
    try {
      ...
      if (!this.calorieNinjasKey) {
        console.warn('⚠️ CalorieNinjas API key not configured, using basic nutrition estimates instead')
  ...
  transformMealDBRecipe(meal) {
    ...
    const baseRecipe = {
      ...
      slug: meal.strMeal ? meal.strMeal.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') : null,
      nutrition: {
        calories: 0,
        protein: 0,
        fat: 0,
