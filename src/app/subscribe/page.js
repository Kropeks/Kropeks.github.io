...
export default function SubscribePage() {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        if (hasActiveStatus) {
          ...
          setCurrentPlan(isYearlyPlan ? 'yearly' : 'monthly');

          if (isYearlyPlan) {
            setCanUpgrade(false);
            setInfoMessage('You have an active Premium Yearly subscription. You can switch to the monthly plan once this term ends.');
          } else {
            setCanUpgrade(true);
            setSelectedPlan('yearly');
            setInfoMessage('You already have an active Premium Monthly subscription. Upgrade to Premium Yearly for the best value.');
          }
        } else {
  ...
  const handlePlanSelect = (plan) => {
    if (hasActiveSubscription) {
      if (canUpgrade && plan === 'yearly') {
        setSelectedPlan(plan);
        setShowModal(true);
        return;
      }
      if (currentPlan === 'yearly' && plan === 'monthly') {
        setInfoMessage('You can switch to Premium Monthly after your current yearly subscription ends.');
        return;
      }
      if (currentPlan === 'yearly' && plan === 'yearly') {
        setInfoMessage('You are already enjoying the Premium Yearly plan.');
        return;
      }
      setInfoMessage(
        canUpgrade
          ? 'You already have an active Premium Monthly subscription. Choose the yearly plan to upgrade.'
          : 'You already have an active subscription. No further action is required.'
      );
      return;
    }
  ...
  const highlightedPlan = hasActiveSubscription && currentPlan ? currentPlan : selectedPlan;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white sm:text-4xl">
            {hasActiveSubscription ? 'Upgrade' : 'Choose Your Plan'}
          </h1>
        ...
        <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          {/* Premium Monthly */}
          <div
            className={`relative flex flex-col rounded-2xl bg-white dark:bg-gray-800 p-8 transition-shadow ${
              highlightedPlan === 'monthly'
                ? 'border-2 border-olive-500 shadow-lg shadow-olive-500/30 dark:shadow-olive-900/40'
          ...
          >
            ...
            <Button
              ...
              disabled={hasActiveSubscription && currentPlan === 'monthly'}
            >
          ...
          <div className="relative flex flex-col rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-8 shadow-sm">
            <div className="absolute top-0 right-0 -mt-4 mr-6">
              <div className="rounded-full bg-gradient-to-r from-yellow-400 via-amber-500 to-orange-600 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-white shadow-lg shadow-amber-500/40 ring-2 ring-amber-300/60">
                Royal Annual
              </div>
            </div>
            ...
            <Button
              ...
              disabled={hasActiveSubscription && !canUpgrade}
            >
              {currentPlan === 'yearly'
                ? 'Current Plan'
