  ...
  try {
    ...
    switch (type) {
      ...
      case 'filterByArea': {
        ...
        if (!area) {
          return Response.json(
            { error: 'Area parameter is required for filterByArea requests' },
            { status: 400 }
          )
        }
        data = await recipeAPI.getMealsByAreaRawMealDB(area)
        break
      }
      ...
      default:
        return Response.json(
          { error: 'Invalid type. Use "categories", "areas", "ingredients", "filterByArea", "latest", or "test"' },
          { status: 400 }
        )
    }
