...
export default function RecipeDetail() {
  ...
  const fetchRecipe = async () => {
    try {
      ...
      const sanitizedSource = normalizedSource === 'edamam' ? 'mealdb' : normalizedSource
      const initialSources = []

      if (sanitizedSource) {
        initialSources.push(sanitizedSource)
      } else {
        const looksNumeric = Number.isFinite(Number.parseInt(id, 10))
        if (!looksNumeric) {
          initialSources.push('community')
        }
        initialSources.push('mealdb', 'community')
      }
  ...
  const viewSourceRaw = (recipe?.source || resolvedSource || sourceParam || 'community').toLowerCase()
  const viewSource = viewSourceRaw === 'edamam' ? 'mealdb' : viewSourceRaw
