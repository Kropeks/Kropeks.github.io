...
const MemoryGame = ({ onClose, canRecord = false }) => {
  ...
  const generateGuestAlias = useCallback(() => {
  ...
  const saveScore = useCallback(async (name, moves, time) => {
    try {
      const response = await fetch('/api/games/memory/leaderboard', {
        method: 'POST',
        headers: {
  ...
  const handleSubmitScore = async (e) => {
    ...
    if (canRecord && !trimmedName) {
      return;
    }

    const nameToSave = canRecord ? trimmedName : generateGuestAlias();

    try {
      await saveScore(nameToSave, moves, time);
      setShowNameInput(false);
      setPlayerName('');
    } catch (error) {
      // Intentionally swallow to avoid crashing the UI, already logged in saveScore
    }
  };
